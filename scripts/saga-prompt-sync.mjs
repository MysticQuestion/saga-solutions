...
const OUTPUT = new URL('../public/prompts/index.json', import.meta.url);
const LIMIT = Math.max(1, Math.min(Number.parseInt(process.env.SAGA_PROMPT_SYNC_LIMIT || '18', 10) || 18, 50));
...
async function notion(path, options = {}) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Notion request failed (${response.status}): ${body.slice(0, 800)}`);
  }
...
async function main() {
  if (!process.env.NOTION_API_KEY) {
    console.log('NOTION_API_KEY is absent; leaving the existing prompt snapshot unchanged.');
    process.exitCode = 0;
    return;
  }
  ...
  const prompts = await Promise.all(
    (database.results || []).map(async (page) => {
      try {
        ...
        console.warn(`Could not read full body for ${page.id}: ${error.message}`);
  ...
  console.log(`Wrote ${prompts.length} prompt records to public/prompts/index.json.`);
}
